...
`,Q="I",V="J",ee="L",te="O",oe="S",re="T",ae="Z",ne=[Q,V,ee,te,oe,re,ae],se=new Array(16).fill(new Array(16).fill(0)),ie=new Array(16).fill(new Array(16).fill(1)),ce=_.addFolder("graphics"),le=[16,16],pe=e=>e.map((e=>e.map((e=>{return t=e,ne.indexOf(t)>-1?[...de[e](),1]:1===e?[1,1,1,1]:[0,0,0,0];var t})))),de={I:ce.auto("#ff9b0d","I").map(q),J:ce.auto("#497bff","J").map(q),L:ce.auto("#ff5d5d","L").map(q),O:ce.auto("#00f0ff","O").map(q),S:ce.auto("#00ff86","S").map(q),T:ce.auto("#ff74d1","T").map(q),Z:ce.auto("#fffb51","Z").map(q)},ue=_.addFolder("audio").auto(120,"BPM",60,240),me=y(F.scan(((e,t)=>e+t/60*ue()),0,S).map(Math.floor)),fe={I:[[Q,Q,Q,Q]],T:[[re,re,re],[0,re,0]],J:[[0,V],[0,V],[V,V]],L:[[ee,0],[ee,0],[ee,ee]],S:[[0,oe,oe],[oe,oe,0]],Z:[[ae,ae,0],[0,ae,ae]],O:[[te,te],[te,te]]},ve=e=>({height:fe[e].length,width:fe[e][0].length}),he=e=>{const{height:t,width:o}=ve(e),r=16-t,a=16-o;return[Math.floor(Math.random()*r),Math.floor(Math.random()*a)]};var be;const ye=(be=A,e=>(...t)=>e(be(),...t))(k),we=(e,t,o)=>{const[r,a]=o;return[r<0?0:r>e?e:r,a<0?0:a>t?t:a]},ge=[0,-1],xe=[0,1],ke=[-1,0],Ae=[1,0],Oe={playerBoard:{tetronimoes:[{tetronimo:re,position:[0,0]}],holes:[],walls:[],boardType:0},stack:[...h.range(0,7).map((()=>({tetronimoes:[],holes:[],walls:[],boardType:0})))]},$e=e=>{let t=1===e.boardType?ie:se;return e.walls.forEach((e=>{t=U(t,fe[e.tetronimo],e.position,1)})),e.holes.forEach((e=>{t=U(t,fe[e.tetronimo],e.position,0)})),e.tetronimoes.forEach((e=>{t=U(t,fe[e.tetronimo],e.position,e.tetronimo)})),t},Fe=e=>{const t=[];return t.push($e(e.playerBoard)),e.stack.forEach((e=>t.push($e(e)))),t},Ee=(e,t)=>{const o=e.playerBoard.tetronimoes.map((e=>{const o=ye(e.position,t),{width:r,height:a}=ve(e.tetronimo);return n(n({},e),{position:we(16-a,16-r,o)})}));return n(n({},e),{playerBoard:n(n({},e.playerBoard),{tetronimoes:o})})};let je=!1,Ce=Oe,Ie=Fe(Ce);const Me=O({extensions:["OES_texture_float"]}),qe=new $;qe.showPanel(0),document.body.appendChild(qe.dom);const Te=(e=>{const t=(e=>{let t=h.range(0,8).map((()=>e.texture({min:"nearest",mag:"nearest",format:"rgba",type:"float32",shape:le,wrap:"repeat"})));return{update:b.flow((e=>e.map(pe)),(e=>e.map(((e,o)=>t[o].subimage(e))))),draw:z`
    ${Y}
    ${K}
  ...
  `}})(e);return F.on((()=>{t.update(Ie)}),L),F.on((e=>{Ce=((e,t)=>{const o=n(n({},e.stack[0]),{tetronimoes:[...e.playerBoard.tetronimoes,...e.stack[0].tetronimoes],holes:[...e.stack[0].holes],walls:[...e.stack[0].walls],boardType:e.stack[0].boardType});var r;return{playerBoard:o,stack:[...e.stack.slice(1),t%8==0?(r=o.tetronimoes,{tetronimoes:[],holes:r.map((e=>e.tetronimo)).map((e=>({tetronimo:e,position:he(e)}))),walls:[],boardType:1}):{tetronimoes:[],holes:[],walls:[],boardType:0}]}})(Ce,e),Ie=Fe(Ce),je&&(e=>{const t=(new g).toDestination();x(),t.triggerAttackRelease(e%8==0?"C5":"C4","8n")})(e)}),me),R.map((e=>(e.a&&(Ce=Ee(Ce,ge)),e.s&&(Ce=Ee(Ce,Ae)),e.d&&(Ce=Ee(Ce,xe)),e.w&&(Ce=Ee(Ce,ke)),e))).map((e=>{!je&&Object.keys(e).length&&(w(),je=!0),Ie=Fe(Ce)})),e(t.draw)})(Me);F.on((()=>{Me.poll(),Te(),qe.update()}),L);
